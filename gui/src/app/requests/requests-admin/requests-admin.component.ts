...
export class RequestsAdminComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<RequestOverview> =
    new MatTableDataSource<RequestOverview>();

  ngOnInit() {
    this.requestsSubscription = this.requestsService.getAllRequests().subscribe(
      requests => {
    ...
    this.dataSource.sortingDataAccessor = (data, sortHeaderId) => {
      switch (sortHeaderId) {
        ...
        case 'serviceName': {
          ...
          if (
            data.serviceName &&
            data.serviceName.has(this.translate.currentLang)
          ) {
      ...
      for (const part of parts) {
        if (
          !(
            ...
            facilityId.includes(filter) ||
            action.includes(filter) ||
            serviceIdentifier.includes(filter) ||
            status.includes(filter) ||
            requesterId.includes(filter)
          )
        ) {
          return false;
        }
