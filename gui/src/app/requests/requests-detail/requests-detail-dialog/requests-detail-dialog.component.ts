...
export class RequestsDetailDialogComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.data.isApprove) {
      this.translate
        .get('REQUESTS.ARE_YOU_SURE_APPROVE')
        .subscribe(value => (this.promptText = value));
    } else if (this.data.isSetWFC) {
      this.translate
        .get('REQUESTS.ARE_YOU_SURE_SET_WFC')
        .subscribe(value => (this.promptText = value));
    } else if (this.data.isCancel) {
      this.translate
        .get('REQUESTS.ARE_YOU_SURE_CANCEL')
        .subscribe(value => (this.promptText = value));
    } else {
      this.translate
        .get('REQUESTS.ARE_YOU_SURE_REJECT')
        .subscribe(value => (this.promptText = value));
    }
  }
