...
export class FacilityAddAdminComponent implements OnInit, OnDestroy {
  ...
  addAdmin (): void {
    ...
    this.facilitiesService
      .addAdmins(this.facility.id, this.emails)
      .subscribe((response) => {
        this.loading = false
        if (response) {
          this.translate
            .get('FACILITIES.ADD_FACILITY_ADMINS_SUBMITTED')
            .subscribe((successMessage) => {
              this.translate
                .get('FACILITIES.GO_TO_FACILITY_DETAIL')
                .subscribe((goToFacilityMessage) => {
                  ...
                  snackBarRef.onAction().subscribe(() => {
                    this.router.navigate([
                      '/auth/facilities/detail/' + this.facility.id
                    ])
                  })
  ...
  add (event: MatChipInputEvent): void {
    ...
    if (value.length <= 5 || !EMAIL_REGEX.test(value)) {
      this.translate
        .get('FACILITIES.EMAIL_ERROR_CORRECT')
        .subscribe((result) => {
          this.snackBar.open(result, null, { duration: 5000 })
        })
    } else {
