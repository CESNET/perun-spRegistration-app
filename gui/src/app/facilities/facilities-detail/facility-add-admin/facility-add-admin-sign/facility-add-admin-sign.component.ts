...
export class FacilityAddAdminSignComponent implements OnInit, OnDestroy {
    ...
    this.sub = this.route.queryParams.subscribe(
      (params) => {
        ...
        this.facilitiesService
          .addAdminGetDetails(this.hash)
          .subscribe((details) => {
            if (details) {
              this.details = new LinkCode(details)
              this.facilitiesService
                .addAdminGetFacilityDetails(this.details.facilityId)
                .subscribe((response) => {
  ...
  private mapAttributes () {
    ...
    this.serviceAttrs = FacilityAddAdminSignComponent.sortItems(
      this.serviceAttrs
    )
  ...
  addAdminConfirm (): void {
    this.facilitiesService.addAdminConfirm(this.hash).subscribe(
      (_) => {
        this.translate
          .get('FACILITIES.ADD_ADMIN_SUCCESS')
  ...
  addAdminReject (): void {
    this.facilitiesService.addAdminReject(this.hash).subscribe(
      (_) => {
        this.translate
          .get('FACILITIES.REJ_ADMIN_SUCCESS')
          .subscribe((successMessage) => {
            this.snackBar.open(successMessage, null, { duration: 5000 })
            this.router.navigate(['/auth'])
          })
