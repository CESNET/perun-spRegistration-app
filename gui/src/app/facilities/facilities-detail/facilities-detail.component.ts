...
export class FacilitiesDetailComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.isUserAdmin = AppComponent.isApplicationAdmin();
    this.translate
      .get('FACILITIES.DETAIL.ADMIN_NOT_REMOVED')
      .subscribe(value => (this.adminRemoveFailText = value));
    this.translate
      .get('FACILITIES.DETAIL.ADMIN_REMOVED')
  ...
  loadMoveToProductionActive(activeRequestId: number) {
    this.requestsService.getRequest(activeRequestId).subscribe(request => {
      this.moveToProductionActive =
        request.action === RequestAction.MOVE_TO_PRODUCTION;
    });
  }
  ...
  moveToProduction(): void {
    this.router.navigateByUrl(
      'auth/facilities/moveToProduction/' + this.facility.id
    );
  }
  ...
  openRemoveAdminDialog(userName: string, userId: number): void {
    this.dialog.open(FacilitiesRemoveAdminDialogComponent, {
      width: '50%',
      data: { parent: this, userName, userId },
    });
  }
  ...
  removeFacilityAdmin(adminToRemoveId: number): void {
    this.facilitiesService
      .removeAdmin(this.facility.id, adminToRemoveId)
      .subscribe(result => {
        this.loading = false;
        if (result) {
          this.snackBar.open(this.adminRemoveSuccessText, null, {
          ...
          this.snackBar.open(this.adminRemoveFailText, null, {
            duration: this.snackBarDurationMs,
          });
        }
      });
  ...
  regenerateClientSecret(): void {
    ...
    this.facilitiesService.regenerateClientSecret(this.facility.id).subscribe(
      data => {
  ...
  private mapAttributes() {
    ...
    this.facilityAttrsService = FacilitiesDetailComponent.sortItems(
      this.facilityAttrsService
    );
    this.facilityAttrsOrganization = FacilitiesDetailComponent.sortItems(
      this.facilityAttrsOrganization
    );
    this.facilityAttrsProtocol = FacilitiesDetailComponent.sortItems(
      this.facilityAttrsProtocol
    );
    this.facilityAttrsAccessControl = FacilitiesDetailComponent.sortItems(
      this.facilityAttrsAccessControl
    );
  }
  ...
  private loadAudit(id: number) {
    ...
    this.auditService.getAuditsForService(id).subscribe(
      audits => {
