...
export class FacilitiesEditComponent implements OnInit {
  ...
  ngOnInit() {
    this.translate
      .get('FACILITIES.NEW_VALUES_ERROR_MESSAGE')
      .subscribe(value => (this.errorText = value));
    this.translate
      .get('REQUESTS.COULD_NOT_SUBMIT_EMPTY')
    ...
    this.translate
      .get('FACILITIES.ERROR_REQUEST_ALREADY_EXISTS')
  ...
  submitRequest() {
    ...
    if (this.facility.activeRequestId != null) {
      this.snackBar.open(this.errorRequestAlreadyExists, null, {
        duration: 6000,
      });
      return;
    }
    ...
    this.facilityService
      .changeFacility(this.facility.id, perunAttributes)
      .subscribe(reqId => {
        if (reqId === null) {
          this.loading = false;
          this.snackBar.open(this.failedActionText, null, {
  ...
  private getAttributes(): void {
    this.sub = this.route.params.subscribe(params => {
      this.facilityService.getFacilityWithInputs(params.id).subscribe(
        facility => {
  ...
  private filterAndSortArrays() {
    this.serviceAttrs = FacilitiesEditComponent.filterAndSort(
      this.serviceAttrs
    );
    this.organizationAttrs = FacilitiesEditComponent.filterAndSort(
      this.organizationAttrs
    );
    this.protocolAttrs = FacilitiesEditComponent.filterAndSort(
      this.protocolAttrs
    );
    this.accessControlAttrs = FacilitiesEditComponent.filterAndSort(
