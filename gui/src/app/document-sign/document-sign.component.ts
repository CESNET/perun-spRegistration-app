...
export class DocumentSignComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.routeSub = this.route.queryParams.subscribe(params => {
      if (params.code) {
        ...
        this.requestDetailsSub = this.fService
          .getRequestDetailsWithHash(this.code)
          .subscribe(request => {
            this.facilitySignatureSub = this.fService
              .getFacilitySignature(request.facilityId)
              .subscribe(
                facility => {
  ...
  approveRequest(): void {
    this.fService.approveTransferToProduction(this.code).subscribe(_ => {
      this.translate
        .get('FACILITIES.DOCUMENT_SIGN_SUCCESS')
